...
vehicle = [cols for cols in mvc.columns if 'vehicle' in cols]
...
top10_vehicles = stacked.value_counts()[:10]
...
for v in range(1,6):
    v_col = 'vehicle_{}'.format(v)
    c_col = 'cause_vehicle_{}'.format(v)
    temp_v = (mvc[v_col].isnull() & mvc[c_col].notnull())
    temp_c = (mvc[c_col].isnull() & mvc[v_col].notnull())
    
    mvc[v_col] = mvc[v_col].mask(temp_v,'Unspecified')
    mvc[c_col] = mvc[c_col].mask(temp_c,'Unspecified')
...
for cols in location_cols:
    mask = mvc[cols].isnull()
    mvc[cols] = mvc[cols].mask(mask, sup_data[cols])
